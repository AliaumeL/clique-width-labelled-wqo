...
\AP Historically, this approach was used to prove that some classes of graphs are
\kl{labelled-well-quasi-ordered}, see for instance the results of
\cite{DING92,DRT10}. Unfortunately, the \kl{composition ordering} on trees is
often way stricter than the \kl{induced subgraph} ordering. It was
...
\intro{$k$-neighbours} if $\spt(x \colon y) \geq k$. Being \kl{$k$-neighbours}
is an equivalence relation on the nodes of the tree having value $k$ in the
split.
\maelin{This is false, for example if a,b,c, are such that a is the lca of b and c; we can have a,b and a,c are k-nbr, but not b,c k-nbd. It should be defined branch-wise.}
 A \kl{split} induces a hierarchical structure on the tree based on the
\kl{$k$-neighbourhoods}. We illustrate the situation on a \emph{branch} of a
tree in \cref{split-on-branch:fig}, but in general \kl{$k$-neighbourhoods} can
span several branches of the tree. We will refer to the value of $\spt(x)$ as
the \reintro{split depth} of the node $x$.
...
\maelin{What is a class of $k$-neighbourhood ? I think the way you define it is not clear yet. You use it exclusively on branch, I think you should define it only on branch, but observe that on the whole tree, the equivalence classes intersect in a tree-like manner.}
...
not change when inserting new nodes in the section between $z_2$ and $z_3$ as
...
\begin{definition}[{\cite[Definition 3.3]{DERSHOWITZ200380}}]
  ...
  \begin{enumerate}
    ...
    \item satisfies the root gap property:
...
induced by \kl{forward Ramseyan splits}.
...
One other key property of the \kl{gap-embedding} ordering is that it is
a \kl{well-quasi-ordering}.
...
ordering that will ensure that the interpretation $\someInterp$ is
order-preserving from trees to graphs. In particular, this modification will
take into account the edge-labelling of the trees using the monoid $M$, that 
are not considered in the \kl{gap-embedding} ordering.
...
\maelin{I think there is something wrong with this definition. "that
	for every $x \treelt[\t] z_1 \treelt[\t] y$ such that 
	$\spt(x \colon z_1) > k$, $\spt(z_1) = k$" there is only one such $z_1$, Moreover, you assume $\marking(z_1) = \marked$ afterward.}
  ...
  if $x$ and $y$ are \kl{marked nodes} and $z_1$ is not a \kl{marked node}, then there exists $z_2, z_3$
  such that 
...
We create an infinite \kl{antichain} by 
...
attention to trees that make a limited use of the gluing property. A
\kl{$L$-bounded marked nested tree} is a \kl{marked nested tree} such that any
path of non \kl{dummy} nodes has length at most $L$. As we will see in the
following \cref{thm:marked-nested-trees-wqo}, the class of \kl{$L$-bounded
well-marked nested trees} is \kl{well-quasi-ordered} under the \kl{marked
gap-embedding} ordering when labelled over a \kl{well-quasi-ordered} set.
...
\begin{proof}
  ...
  Since we know that the class of \kl{nested trees} labelled over a
  \kl{well-quasi-ordered} set is \kl{well-quasi-ordered} under the \kl{gap-embedding} ordering
  by \cref{thm:gap-embedding-wqo}, we will immediately conclude that 
  the class of \kl{$L$-bounded well-marked nested trees} is also \kl{well-quasi-ordered}
  under the \kl{marked gap-embedding} ordering.
