...
\maelin{Are $\mu(T[x \colon y])$ and $\tlbl{T}{x}{y}$ equal? Maybe say that in the section $\mu$ is fixed and define $\tlbl{T}{x}{y}$ in consequence. }
...
to $k$, and the resulting tree is still \kl{forward Ramseyan}. \maelin{bigger ? je trouve ce passage un peu dur à suivre, peut-être motiver mieux la raison de pourquoi ne change pas $\tlbl{\t}{x}{y}$} A fortiori,
