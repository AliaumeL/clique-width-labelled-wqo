...
\begin{proof}
  ...
  Indeed, by \cref{item:gap-embedding:gap},
  ...
  \cref{item:gap-embedding:marking} that
  $\spt_1(x:z_1) = \spt_2(h(x):h(z_1)) > k$,
  $\spt_1(z_1:z_2) = \spt_2(h(z_1):h(z_2)) > k$.

  \textbf{Todo: use the fact that gap is gluing to 
  conclude about the strict things.}

  Now, by induction hypothesis, we have
  $\tlbl{\t_1}{z_1}{z_2} = \tlbl{\t_2}{h(z_1)}{h(z_2)}$.
  Furthermore, by \cref{item:gap-embedding:neighbourhood},
