...
The interpretation works as follows: given a tree $\aTree$ whose edges are labelled
...
the triple $(\tlbl{\aTree}{\treeRoot}{\lca(x,y)},
\tlbl{\aTree}{\lca(x,y)}{x}, \tlbl{\aTree}{\lca(x,y)}{y})$ is in $P$.
...
\AP Historically, this approach was used to prove that some classes of graphs
are \kl{labelled-well-quasi-ordered}, see for instance the results of
\cite{DING92,DRT10}. Unfortunately, the \kl{composition ordering} on trees is
often way stricter than the \kl{induced subgraph} ordering. It was observed
...
happens for a precise choice of $P$, we will develop a finer understanding of
...
\intro{$k$-neighbours} if $\spt(x \colon y) \geq k$. For every branch of the
tree, being \kl{$k$-neighbours} is an equivalence relation on the nodes of the
tree having value $k$ in the split belonging to said branch. A \kl{split}
induces a hierarchical structure on every branch of the tree, based on the
...
satisfies the equation $e \cdot e = e$. However, note that $\tlbl{\t}{x}{y}$
and $\tlbl{\t}{x'}{y'}$ may be different \kl{idempotents}.
...
= k$ and there exist three nodes $z_1, z_2, z_3$ such that $x \treeleq[\t] z_1
\treelt[\t] z_2 \treeleq[\t] z_3 \treeleq[\t] y$ and $\spt(z_1) = \spt(z_2) =
\spt(z_3) = k$, and $\spt(x\colon z_1) > k$, $\spt(z_1 \colon z_2) > k$, and $\spt(z_3\colon y) >
k$. The idea being that $x$ and $y$ are \emph{separated} by at least
one \kl{$k$-neighbourhood}.
In the case of \kl{independence at level $k$}, we can use the property of
\kl{forward Ramseyan splits} to 
compute $\tlbl{\t}{x}{y}$ without ever looking at the section
between $z_2$ and $z_3$, leading to a fast (and first-order definable) computation.
We refer to \cite[Lemma 3]{COLC07} for more details on this computation.
...
using $M$, there exists a forward Ramseyan split of height $N$ for $\aTree$. We can
...
illustrated in \cref{fast-computation:fig}, and assuming that $z_2 \neq z_3$,
the value of $\tlbl{\t}{x}{y}$ does not change when inserting new nodes in the
section between $z_2$ and $z_3$ as long as these new nodes are assigned a value
of the split bigger than or equal to $k$, and the resulting tree is still
\kl{forward Ramseyan}. A fortiori, whether there is an edge between two leaves
of $x$ and $y$ in the resulting graph is not influenced by such insertions.
This will guide our definition of a suitable ordering on trees in the upcoming
\cref{sec:marked-nested-trees}.
...
\AP In the following \cref{fact:gap-embedding-gaps} we illustrate why
\kl{gap-embeddings} are well-suited for our purpose: they respect the gaps
induced by \kl{forward Ramseyan splits}. One other key property of the
\kl{gap-embedding} ordering is that it is a \kl{well-quasi-ordering} as
recalled in \cref{thm:gap-embedding-wqo}.
...
\AP We will now devise a suitable modification of the \kl{gap-embedding}
...
\begin{definition}
  ...
  $\spt(x\colon z_1) > k$, $\spt(z_1) = k$, and  $\spt(z_1\colon y) \geq k$ for some $k \in \set{1, \dots, N}$,
  if $x$ and $y$ are \kl{marked nodes}, then there exists $z_2, z_3$
  such that 
  $x \treelt[\t] z_1 \treelt[\t] z_2 \treeleq[\t] z_3 \treeleq[\t] y$,
  $\spt(z_1) = \spt(z_2) = \spt(z_3) = k$, 
  $\spt(x\colon z_1) > k$, $\spt(z_1\colon z_2) > k$, $\spt(z_2\colon z_3) \geq k$, 
  and $\spt(z_3\colon y) \geq k$,
...
\kl{well-quasi-order} because of the gluing condition of
\cref{item:gap-embedding:gluing}.
We can create an infinite \kl{antichain} by 
...
\kl{$L$-bounded marked nested tree} is a \kl{marked nested tree} such that any
