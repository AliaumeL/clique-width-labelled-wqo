...
\treelt[\t] y$, we define $\spt(x : y)$ to be the minimal value of
$\spt(z)$ for $x \treelt[\t] z \treelt[\t] y$, and $N+1$ otherwise. In
particular, this does not count the nodes $x$ and $y$ themselves.
\maelin{Replacement suggestion for previous sentence: In particular, if $y$ is a child of $x$, then $\spt(x : y) = N+1$.}

\maelin{Define more properly $k$-neighbourhoods. I'm not sure how do you define it, is it dependent of one node like the k-neighborhood of $u$? Maybe be more precise about what ou mean by hierarchical. Does these nbd is for branches only or for the whole tree ? }
\AP Two elements $x \treelt[\t] y$ such that $\spt(x) = \spt(y) = k$ are
\intro{$k$-neighbours} if $\spt(x : y) \geq k$. Note that a \kl{split}
...
\begin{figure}
  ...
  arcs of each color.
  \maelin{Choose more different colors, + add the arcs between the first and last 1, and same for the 2. }
  }
  \label{split-on-branch:fig}
\end{figure}

\maelin{It should be for every branch of the tree no ?}
...
k$. 
\maelin{Maybe explain a bit more why you need three vertices and why only $z_2 \treeleq[\t] z_3 $. This is a bit surprising}
In the case of \kl{independence at level $k$}, we can use the property of
\kl{forward Ramseyan splits} to replace the middle part\mael{define middle part} of the path from $x$ to
...
\begin{proof}
  ...
  such that if $f(\t_1, \spt_1, \marking_1) \gemb f(\t_2, \spt_2, \marking_2)$
  then there exists a \kl{gap-embedding} from
  $(\t_1, \spt_1, \marking_1)$ to
  $(\t_2, \spt_2, \marking_2)$. This is done by encoding markings and 
  conditions \cref{item:gap-embedding:gluing} of \cref{def:gap-embedding}
  into node labels and modifying the splits accordingly.


  \maelin{Maybe you can present this proof better. Maybe say something along the lines: we will reduce to the case of node-labelled trees equipped with forward Ramseyan splits.}
  ...
  Since $M$ is finite, the new labelling is still over a \kl{well-quasi-ordered} set. \mael{ref}
...
\begin{proof}
  ...
  on the above paths. Large inequalities 
