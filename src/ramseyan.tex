...
\begin{figure}
  ...
  the arcs in \textcolor{A2}{light red} connect $2$-neighbours,
  and the arcs in \textcolor{B5}{light green} connect $3$-neighbours.
  }
...
\maelin{I think there is something wrong with this definition. "that
	...
	$\spt(x:z_1) > k$, $\spt(z_1) = k$" there is only one such $z_1$, Moreover, you assume $\marking(z_1) = \marked$ afterward.}
  ...
  $\spt(x:z_1) > k$, $\spt(z_1:z_2) > k$, $\spt(z_2:z_3) \geq k$, 
  and $\spt(z_3:y) \geq k$,
...
\begin{proof}
  ...
  If $\spt(x:y) = N+1$, then $y$ is a child of $x$.
  ...
  $\spt_1(x:z_1) > k$, $\spt_1(z_1:z_2) > k$, $\spt_1(z_2:z_3) \geq k$, and $\spt_1(z_3:y) > k$, 
