...
\maelin{Put the following in a lemma/theorem}
\AP 
...
\begin{definition}
  ...
  such that 
  $x \treelt[\t] z_1 \treelt[\t] z_2 \treeleq[\t] z_3 \treeleq[\t] y$, \mael{$z_3 \treeleq[\t] y$ correct ?}
  $\spt(z_1) = \spt(z_2) = \spt(z_3) = k$, and 
  $\spt(x:z_1) > k$, $\spt(z_1:z_2) > k$, and $\spt(z_3:y) \geq k$, \mael{what about $\spt(z_2:z_3)$ ?} 
...
\begin{proof}
  ...
  If $i = 0$\mael{should be $i=1$} (i.e., $y$ is the first node of a maximal path of non-\kl{dummy} nodes),
...
\maelin{Je mettrais ça avant le théorème, et pas forcément sous forme d'un lemme, tu peux juste dire que la famille des chemins dont les noeuds sont marqué n'est pas wqo à cause du gluing.}
\begin{lemma}
