...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    if $\marking_1(x) \in \set{\marked, \separating}$,
    and $y$ is the parent of $x$ in $\t_1$,
    then $h(y)$ is the parent of $h(x)$ in $\t_2$.
...
\begin{proof}
  ...
  strictly greater than $k$, and we conclude by the previous argument.\footnote{The case where there are no intermediate nodes between $u$ and $v$ is given directly by the definition of \kl{gap-embedding}.
  node between $u$ and $v$ happens if $v$ is the immediate child of $u$, 
