...
\begin{lemma}
  ...
  \maelin{Let $\Cls = \seqof{G^r}[r \geq 1]$ be a periodic sequence of graph forming an antichain. Then $\Cls$ \kl{existentially interprets}
  	the class of all finite paths. }
\end{lemma}
\begin{proof}
...
\end{claimproof}
  Assuming $r$ is sufficiently larger than $|V(G)|$, there is two vertices $u_i$ and $u_j$, $i<j$, in the spanning path of $H^r$, which are copies of $u\in V(G)$. Take these two vertices to be such that the distance from $u_i$ to $u_j$ is the shortest possible in $H^r$. Let $t = j-i$, this is the period of $P$, i.e. the number of copies of $G$ it have to go through until it come back to a copy of the same vertex.
  Let $Q = u^1,u^2,\dots, u^p = P-\{u_j\}$.   In particular, $Q$ is isomorphic to a spanning path of $H^{t}$ and $Q^\ell$ is isomorphic to a spanning path of $H^{t\ell}$ (where the last vertex of $P_i$ is made adjacent with the first vertex of $P_{i+1}$).
  ...
  \maelin{update here}
  If there is a backward arc $uv \in E(H^r)$ in $P$ with  $u \in P_i$ and $v\in P_j$ with $i>j$, then by \cref{clm:regularbackward} this arc exists between every copy of $u$ and $v$ where $v$ is an earlier copy than the one of $u$. 
...
\begin{corollary}
  ...
  If a hereditary class of graphs $\Cls$ \mael{of bnd cw?} is not \kl{$\forall$-well-quasi-ordered},
...
\end{corollary}
\begin{proof}
	for ward direction follows from \cref{lem:paths-transduction}, reverse is trivial.
\end{proof}
...
\AP
Let us recall the definition of \kl{weak periodic antichain} 
from \cite[Conjecture 2]{ALM17}: given a finite alphabet $\Sigma$, and 
...
if and only if $(w[i], w[i + 1]) \not\in S$.  In this setting, a \intro{(weak) periodic sequence} is given by a finite word $u
