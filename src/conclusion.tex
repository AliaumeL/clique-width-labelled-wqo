...
\begin{quote}
path we obtain an infinite antichain.  Moreover, being \kl{$\ell$-wqo}
entails being \kl{$k$-wqo} for all $k\leq\ell$ and similarly being
\kl{$n^-$-wqo} entails being \kl{$m^-$-wqo} for all $m\leq n$.
Finally, if $\mathscr C$ is \kl{$2$-wqo} then it is \kl{$n^-$-wqo} for
all~$n$ \cite[Lemma~5.2]{braunfeld21}.
\end{quote}
...
\begin{conjecture}
  ...
  \kl{well-quasi-ordered} by the \kl{induced subgraph} relation is \kl{monadically dependent}.
\end{conjecture}

\begin{conjecture}
  \label{conj:cwqo-2minwqo}
  Every \kl{hereditary class} of graphs that is 
  \kl{$2^-$-well-quasi-ordered} by the \kl{induced subgraph} relation is \kl{$2$-well-quasi-ordered} by the \kl{induced subgraph} relation.
\end{conjecture}
