...
obtained from $H$ by deleting vertices (and the edges adjacent to them)\mael{unecessary comment}. Unlike
...
\maelin{We may avoid mentionning everytime finite graphs, as we only consider finite ones}
...
5.1]{Kuske06}). However, there are high hopes that a finer notion of
...
\begin{example}
  ...
  since any \kl{$X$-labelled clique} can be identified with a finite multiset of labels from $X$,
...
well-quasi-orderings: clasical results such as Higman's lemma \cite{HIG52} or
...
are defined as structures labelled by $(\mathbb{N}, \leq)$.


\AP Even for classes of finite undirected graphs, these conjectures remain
open. Several works that date back to the 90s have idendified structural
...
(using NLC graph expressions), and identifying which ones are
\kl{$2$-well-quasi-ordered}. In this context, they verified Pouzet's
conjectures, but were unable to generalise their results\mael{to what?} \cite[Conjecture
4]{DRT10}. All the classes considered in \cite{DRT10} have so-called
\emph{bounded clique-width},\footnote{This will be defined formally in 
\cref{sec:prelims}} and it is believed that it is enough to understand
hereditary classes of bounded clique-width to solve these conjectures in full
generality, as stated in the following conjecture.
...
formulas. This lead to the following conjecture.
...
were called \kl(weak){periodic antichains} in \cite[Section 7]{ALM17}.
...
this is not formally stated in the paper. It was conjectured \cite[Section
5]{LOPEZ24} that these techniques could be refined to work on classes of
...
that answers positively to the conjectures of \cite{LOPEZ24}, and completely
solves both Pouzet's conjectures for hereditary classes of bounded clique-width
(see \cref{main:corollary}).
...
\begin{theorem}
  ...
  \begin{itemize}
    \item The class $\Label{X}{\Cls}$ is well-quasi-ordered under the labelled induced subgraph relation,
      where $X$ is a two-element antichain,
    \item 
      The class $\Label{X}{\Cls}$ is well-quasi-ordered under the labelled induced subgraph relation, \mael{missing text}
    \item 
      There exists a way to endow every graph of $\Cls$ with a total order on its vertices,
      such that 
      $\Label{X}{\Cls}$ is well-quasi-ordered under the (ordered) labelled induced subgraph relation,
      for every well-quasi-ordering $(X, \leq)$.
  \end{itemize}
\end{theorem}
...
From our proof techniques, we also derive nice obstructions to being
\kl{$2$-well-quasi-ordered}, that yield the equivalences stated in
...
The results of \cref{main:corollary} extends to non-hereditary classes of
bounded clique-width graphs, but for the sake of clarity and space, we only
state them on hereditary ones.
...
\begin{example}
  ...
  Let $\Cls$ be the class of graphs that are complete binary trees,
  i.e., acyclic graphs where every vertex has degree at most $3$, and a single vertex has degree $2$.\mael{the i.e. is not necessary}
  Then, $\Cls$ is of bounded clique-width and is not $2$-well-quasi-ordered, 
  but: every class $\Cls[D] \subseteq \Cls$ of bounded linear clique-width
  is finite hence $2$-well-quasi-ordered. Since \kl{periodic antichains}
  have bounded linear clique-width, there is no \kl{periodic antichain} in $\Cls$.
  Note that $\Cls$ existentially transduces the class of all finite paths
  by selecting one leaf and following the path to the root, but 
  one can show that $\Cls$ does not existentially \emph{interpret}
