...
over which verification algorithms can be designed~\cite{ABDU96,ABDU98}. 
One appealing feature of WQOs is that they are closed under various operations,
...
\maelin{This sentence does not make sense: `the closure' is not also known as `Higman's lemma'. (That lemma is the statement that the given closure is a WQO.)
\\
I agree that this sentence does not make sense. Do you mean that wqo are closed under taking finite words of them ?
}
...
all finite cliques. 
Distinguishing which classes of (finite) graphs are
...
\maelin{"While Pouzet's 1972 work is indeed seminal, this sentence does Kruskal and Nash-Williams a disservice, as they were working on similar ideas in the 1960s." \\
Il parle du tree theorem de kruskal~\cite{KRUSK60,NASH63} là ? où il y a plus ?}
...
\maelin{(re)move this paragraph ? (to contributions, after/before thm6)}
...
\maelin{Do we want to sell this as a corollary and not a theorem, this is our main result}
...
\maelin{I don't see why we prove the conjecture of ~\cite{ALM17}. "Conjecture 2. There is a function $f : N \to N $ such that the class X of graphs defined by a finite collection C of forbidden induced subgraphs is well-quasi-ordered by the induced subgraph relation if and only if $X$ contains no periodic infinite antichains of period at most $f (t(C ))$."}
