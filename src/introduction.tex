...
From our proof techniques, we also derive nice consequences regarding 
hereditary classes of bounded clique-width graphs. Namely:
...
\begin{theorem}
  ...
  \begin{itemize}
    \item $\Cls$ is not $2$-well-quasi-ordered,
    \item There exists a class $\Cls[D] \subseteq \Cls$
      and $\Cls[D]$ is of bounded linear clique-width is not $2$-well-quasi-ordered,
    \item There exists a \kl{periodic antichain} in $\Cls$,
    \item $\Cls$ existentially transduces the class of all finite paths.
  \end{itemize}
\end{theorem}

Note that some of our results naturally extend  to non-hereditary classes, but
for the sake of clarity we chose to state them in the hereditary case only.
