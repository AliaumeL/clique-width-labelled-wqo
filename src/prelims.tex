...
chaque fois.}
\paragraph*{Graphs} \AP A \intro{graph} is a pair $G = (V, E)$
where $V$ is a set of \intro{vertices} and $E \subseteq V \times V$ is a set of
\intro{edges}. A graph is \intro(graph){undirected} if $(u, v) \in E$ implies
...
\intro{$X$-labelled graph} is a graph equipped with a function $\ell \colon V
...
graphs in $\Cls$, i.e., the class of all \kl{$X$-labelled graphs} $G = (V, E,
...
from $G$ to $H$, i.e., an injective function $f \colon V_G \to V_H$ such that
for all $u, v \in V_G$, $(u, v) \in E_G$ if and only if $(f(u), f(v)) \in E_H$,
and such that for all $v \in V_G$, we have $\ell_G(v) \leq \ell_H(f(v))$. We
...
(i.e., trees where every node has at most one non-leaf child) to finite
undirected graphs. Equivalently, a class of graphs has \kl{bounded linear
