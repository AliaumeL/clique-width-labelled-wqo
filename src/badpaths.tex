...
arbitrarily large \kl{dimension}, and we will leverage this to construct an
...
level $k$ with respect to \kl{compatibility}. This is the content of the
following \cref{lem:finitely-many-boughs}. Then, we will show that actually,
one does not need arbitrarily many \kl{contexts} to witness that a \kl{bough}
is \kl{bad}. This is the content of \cref{lem:finitely-many-contexts}.
Combining these two lemmas, we will extract an infinite sequence of \kl{bad
boughs} of the same level that are pairwise \kl{compatible}, and that are
\kl{bad} in the same \kl{context}. Finally, we will leverage this to construct
...
\begin{lemma}
  ...
  up to \kl{compatibility}.
\end{lemma}
\begin{proof}
  Let $B$ be a \kl{bough} of level $k$ and $C[\square]$ be a \kl{context}.
  ...
  and the \kl{context} $C[\square]$ is also \kl{forward Ramseyan},
  ...
  \begin{enumerate}
    ...
    \item \label{item:comp-fr-below} one considers a node $x$ on the branch defined 
      by the \kl{backbone} of $B$, and $y$ in 
  ...
  all nodes on the branch defined by the \kl{backbone} of $B$ are of 
  ...
  $B$, but not necessarily in the branch defined by the \kl{backbone} of $B$.
...
\end{lemma}
\begin{proof}
  The proof relies on the fact that the \kl{context} $C[\square]$ plays 
  a very little role in witnessing that a \kl{bough} $B$ is
  \kl(bough){bad}. 

  Let $B$ and $H$ be two \kl{compatible boughs} of level $k$,
  and let $C[\square]$ be a \kl{context} such that $\t = C[B]$ 
  ...
  two contexts sharing the same value for the path from the root to $\square$
  ...
  then for any other \kl{context} $C'[\square]$ sharing the same
  ...
  Since there are finitely many possible \kl{contexts} up to this 
