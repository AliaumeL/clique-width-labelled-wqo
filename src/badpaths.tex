...
$\square$ in $\t_{\mathsf{root}}$ by the node $b_0$, the first element of the
\kl{$k$-neighbourhood} defining $B$.
...
\begin{proof}
  ...
  of two \kl{marked nodes} is also \kl(node){marked}. By definition, the graph
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item \label{item:comp-fr-below} one considers a node $x$ on the branch defined 
  ...
  requires nodes to be in the same \kl{$l$-neighbourhood}. Or,
  ...
  ancestor of of $\square$ of level $l$ (if it exists) in $C[\square]$ and
  $\alpha$ be the $\tlbl{\t}{x}{\square}$. Furthermore, let $y$ be a descendent
