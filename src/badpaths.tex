...
As witnessed by \cref{lem:almost-order-preserving}, one obstruction for the
interpretation $\someInterp$ to be order-preserving is is the behaviour of
nodes that are ``too close'' to each other. Let us extract a formal obstruction
for being \kl{$2$-well-quasi-ordered} from this informal idea.
...
\begin{figure}
    ...
    \caption{Partitionning a graph using a \kl{bough}.}
    \label{partitionning-a-graph:fig}
\end{figure}

\AP To compute the presence of an edge between two leaves $x$ and $y$ in
$B(T)$, we only need to know their respective \kl{bough types} the order in
which they appear in the \kl{bough}, and how they are separated in the tree
$T$, as illustrated in \cref{partitionning-a-graph:fig}.

\AP Let us define a \intro{context} $C[\square]$ to be a pair of a tree $A$
